							...
							if(param.getCompletedDateFrom() != null) {
								...
								if(param.getTravelDateType().equalsIgnoreCase("1")) {
									...
									matchFilter.add(new BasicDBObject("$or", Arrays.asList(
											...
											new BasicDBObject("$gte", Arrays.asList("$travel_dates_exp", DateUtil.convertObjectToDate(param.getTravelDateFrom()))))
									));
...
//				Date today = getCalendarDate(0);
		...
		if ((param.getSaleDateFrom() != null || param.getSaleDateTo() != null)
				&& (param.getTravelDateFrom() == null 
						|| param.getTravelDateTo() == null)
				&& (param.getCompletedDateFrom() == null 
						|| param.getTravelOpt() == null || !param.getTravelOpt().isEmpty())) {
			
		} else if ((param.getSaleDateFrom() == null 
				|| param.getSaleDateTo() == null)
				&& (param.getTravelDateFrom() == null || param.getTravelDateTo() == null)
				|| param.getCompletedDateFrom() == null
				|| param.getTravelOpt() == null || !param.getTravelOpt().isEmpty()) {
		...
		aggregationOperations.add(new AggregationOperation() {
			...
			public DBObject toDBObject(AggregationOperationContext context) {
				...
				query.append("from", CollectionName.ATPCO_FOOTNOTE_RECORD_3_CAT_015);
				query.append("let", 
						new BasicDBObject("tbl_no", "$tbl_no")
						.append("sales_dates_start", "$sales_dates_earliest_tktg")
						.append("sales_dates_end", "$sales_dates_latest_tktg")
						);
				query.append("pipeline", Arrays.asList(
						new BasicDBObject("$match", 
								new BasicDBObject("$expr", 
										new BasicDBObject("$and", Arrays.asList(
												new BasicDBObject("$eq", Arrays.asList("$tbl_no", "$$tbl_no")),
												new BasicDBObject("$ne", Arrays.asList("$sales_dates_end", "indef")),
												new BasicDBObject("$gte", Arrays.asList("$sales_dates_end",today))
												))
										)
								)
						));
						...
						new BasicDBObject("$match", 
								new BasicDBObject("$expr", 
										new BasicDBObject("$and", Arrays.asList(
												new BasicDBObject("$eq", Arrays.asList("$tbl_no", "$$tbl_no")),
												new BasicDBObject("$ne", Arrays.asList("$sales_dates_end", "indef")),
												new BasicDBObject("$lt", Arrays.asList("$sales_dates_end",today))
												))
										)
								)
						));
